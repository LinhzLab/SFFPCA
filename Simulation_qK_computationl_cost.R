...
SFFPCA <- function(q1,K1,Kstep,seed) {
  q <- q1
  K <- K1
  set.seed(seed)
  ...
  aa <- system.time({
    for (kstep in 1 : Kstep) {
      ...
      for (lll in 1 : 1) {
        ...
        wtB <- solve(2 * wtH + mu * D+0.0*diag(p*q)) %*% (2 * wtH_X_minus_fH - E )
        B <- t(matrix(wtB, q, p))
      ...
      f <- Bs %*% t(Alpha)
      for (t in 1 : T) H[, , t] = X[, , t] %*% (f) %*% solve( t(f) %*% (f) +0.01*diag(q) )
...
# q = 2,5,10,20
# K = 2,5,10,20
cost_qK <- array(0, c(4, 4, 10))
q_seq <- c(2,5,10,20)
K_seq <- c(2,5,10,20)
for (q_index in 1 : 4) {
  for (K_index in 1 : 4) {
    for (seed in 1 : 10) {
      cost_qK[q_index, K_index, seed] <- SFFPCA(q_seq[q_index], K_seq[K_index], 10, seed)[1]
      
    }
    #print(c(q_index,K_index))
  }
}


cost <- matrix(0, 4, 4)
for (seed in 1 : 10) cost  <- cost  + cost_qK[,,seed] / 10
print(cost)